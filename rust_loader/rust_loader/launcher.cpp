...
HANDLE launcher::LaunchHostProcess(std::string host_proc)
{
	...
	if (!CreateProcessA(
		NULL,
		const_cast<char*>(host_proc.c_str()),
		NULL,
		NULL,
		FALSE,
		CREATE_SUSPENDED,
...
bool launcher::ManualMapExe(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections)
{
...
#pragma optimize( "", off )
void __stdcall launcher::Shellcode(MANUAL_MAPPING_DATA* pData) {
	if (!pData) {
	...
	auto _ExeMain = reinterpret_cast<f_EXE_ENTRY_POINT>(pBase + pOpt->AddressOfEntryPoint);
